...
| [convertToLocalClientTime](#converttolocalclienttimetimevalue--localclienttimejavascriptapioutlookofficelocalclienttime) | Methode |
...
| [displayNewMessageForm](#displaynewmessageformparameters) | Methode |
...
Derzeit sind die unterstützten Ereignistypen `Office.EventType.ItemChanged` und `Office.EventType.OfficeThemeChanged`.
...
|`restVersion`| [Office.MailboxEnums.RestVersion](/javascript/api/outlook/office.mailboxenums.restversion)|Ein Wert, der die Version der Outlook-REST-API angibt, die zum Abrufen der Element-ID verwendet wurde.|
...
Typ: [LocalClientTime](/javascript/api/outlook/office.LocalClientTime)
...
|`restVersion`| [Office.MailboxEnums.RestVersion](/javascript/api/outlook/office.mailboxenums.restversion)|Ein Wert, der die Version der Outlook-REST-API angibt, mit der die konvertierte ID verwendet wird.|
...
|Name| Typ| Beschreibung|
|---|---|---|
|`input`| [LocalClientTime](/javascript/api/outlook/office.LocalClientTime)|Der zu konvertierende Wert für die lokale Uhrzeit.|
...
Mit der `displayNewMessageForm`-Methode wird ein Formular geöffnet, mit dem der Benutzer eine neue Nachricht erstellen kann. Wenn Parameter angegeben wurden, werden die Felder im Nachrichtenformular automatisch mit dem Inhalt der Parameter ausgefüllt.
...
|Name| Typ| Beschreibung|
|---|---|---|
| `parameters` | Object | Ein Wörterbuch mit Parametern, die die neue Nachricht beschreiben. |
| `parameters.toRecipients` | Array.&lt;String&gt; &#124; Array.&lt;[EmailAddressDetails](/javascript/api/outlook/office.emailaddressdetails)&gt; | Ein Array aus Zeichenfolgen mit den E-Mail-Adressen oder ein Array mit einem `EmailAddressDetails`-Objekt für jeden der Empfänger im Feld „An“. Das Array darf maximal 100 Einträge enthalten. |
| `parameters.ccRecipients` | Array.&lt;String&gt; &#124; Array.&lt;[EmailAddressDetails](/javascript/api/outlook/office.emailaddressdetails)&gt; | Ein Array aus Zeichenfolgen mit den E-Mail-Adressen oder ein Array mit einem `EmailAddressDetails`-Objekt für jeden der Empfänger im Feld „Cc“. Das Array darf maximal 100 Einträge enthalten. |
| `parameters.bccRecipients` | Array.&lt;String&gt; &#124; Array.&lt;[EmailAddressDetails](/javascript/api/outlook/office.emailaddressdetails)&gt; | Ein Array aus Zeichenfolgen mit den E-Mail-Adressen oder ein Array mit einem `EmailAddressDetails`-Objekt für jeden der Empfänger im Feld „Bcc“. Das Array darf maximal 100 Einträge enthalten. |
| `parameters.subject` | Zeichenfolge | Eine Zeichenfolge mit dem Betreff für die Nachricht. Die Zeichenfolge ist auf maximal 255 Zeichen beschränkt. |
| `parameters.htmlBody` | Zeichenfolge | Der HTML-Text der Nachricht. Der Textinhalt darf maximal 32 KB umfassen. |
| `parameters.attachments` | Array.&lt;Object&gt; | Ein Array mit JSON-Objekten, die Datei- oder Elementanlagen sind. |
| `parameters.attachments.type` | Zeichenfolge | Gibt den Typ der Anlage an. Muss `file` für eine Dateianlage oder `item` für eine Elementanlage sein. |
| `parameters.attachments.name` | Zeichenfolge | Eine Zeichenfolge, die den Namen der Anlage mit bis zu 255 Zeichen enthält.|
| `parameters.attachments.url` | Zeichenfolge | Wird nur verwendet, wenn `type` auf `file` gesetzt ist. Der URI des Speicherorts für die Datei. |
| `parameters.attachments.isInline` | Boolean | Wird nur verwendet, wenn `type` auf `file` gesetzt ist. Wenn `true`: Zeigt an, dass die Anlage inline im Nachrichtentext angezeigt wird und nicht in der Anlagenlisten angezeigt werden soll. |
| `parameters.attachments.itemId` | Zeichenfolge | Nur verwendet, wenn `type` auf `item` festgelegt ist. Der EWS-Element-Id der vorhandenen e-Mail, den Sie die neue Nachricht zuordnen möchten. Dies ist eine Zeichenfolge bis zu 100 Zeichen. |
