...
Das Objekt RequestContext vereinfacht die Anforderungen an die Visio-Anwendung. Da der Rahmen für Entwickler und die Visio-Online-Anwendung in zwei verschiedenen Iframes ausführen, ist das RequestContext-Objekt (Kontext im nächsten Beispiel) erforderlich, um Zugriff auf Visio und damit zusammenhängende Objekte wie Seiten und -Shapes aus der Rahmen für Entwickler zu erhalten.
...
Die **Visio.run()**-Methode enthält eine Reihe von Anweisungen. Als Teil dieses Batches wird ein Proxyobjekt erstellt, das auf Shapes im aktiven Dokument verweist.
